...
__global__ void reduce(int *g_idata, int *reduced_val, int num_elems){
	...
	if(tid == 0)	*reduced_val = res[0];
}


int main(){
	int *h_data, *d_data, *reduced_val;
	int result;
	int num_elems = MAXN;
	...
	cudaMalloc(&reduced_val, sizeof(int));
	cudaMemcpy(d_data, h_data, size, cudaMemcpyHostToDevice);

	cudaEventRecord(start);
	reduce<<<dimGrid, dimBlock, threads_per_block*sizeof(int)>>>(d_data, reduced_val, num_elems);
	cudaEventRecord(stop);
	cudaMemcpy(&result, reduced_val, sizeof(int), cudaMemcpyDeviceToHost);
	cudaEventSynchronize(stop);
	float exec_time = 0;
	cudaEventElapsedTime(&exec_time, start, stop);

	printf("Cuda Reduction result: %d\n",result);
	printf("Actual answer: %d\n",get_sum(h_data, num_elems));
